<?php
class Listings_New_Block extends Template_Block {
    function content($limit = null) {
        if(is_null($limit)) {
            $limit = intval($this->PMDR->getConfig('block_listings_new_number'));
        }
        if($limit)  {
            $block_listings_new_template = $this->PMDR->getNew('Template',PMDROOT.TEMPLATE_PATH.'blocks/block_listings_new.tpl');
            $block_listings_new_template->cache_id = 'block_listings_new'.$limit;
            $block_listings_new_template->expire = 900;
            if(!$block_listings_new_template->isCached()) {
                ...
                if(!($block_listings_template->isCached())) {
                    ...
                    if(is_array($results) AND sizeof($results) > 0) {
                        foreach($results as $key=>$value) {
                            ...
                            $results[$key]['details'] = $this->PMDR->get('Dates_Local')->formatDateTime($value['date']);
                ...
                $block_listings_new_template->set('listings',$results);
                $block_listings_new_template->set('content',$block_listings_template);
            }
            return $block_listings_new_template;
        }
    }
}
?>