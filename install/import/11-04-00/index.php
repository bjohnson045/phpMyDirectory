...
$template_content = $PMDR->getNew('Template',PMDROOT.'/install/import/11-04-00/index.tpl');

$cant_detect = false;

$form = $PMDR->get('Form');
$form->addFieldSet('import',array('legend'=>'Import'));
$defaults_file = globr(substr(PMDROOT,0,strrpos(PMDROOT,'/',strrpos(PMDROOT,'/'))),'defaults.php');
foreach($defaults_file as $key=>$file) {
    $defaults_content = file_get_contents($file);
    if(!preg_match('/\$DB\[\'TABLE_PREFIX\'\] = \'(.{0,15})\';/',$defaults_content,$matches) OR !preg_match('/\$db_users/',$defaults_content)) {
...
if(count($defaults_file) == 1) {
    ...
    $cant_detect = true;
    $content = 'We were unable to detect your old version.  Please ensure the new version is installed in a sub folder under your v11 installation or enter the path below.';
...
if($form->wasSubmitted('submit')) {
    ...
    if(!$form->validate()) {
        ...
        $upgrade_data['table_prefix'] = $matches[1];
        $upgrade_data['PMDROOT'] = $_POST['path'];
        $_SESSION['upgrade_data'] = $upgrade_data;
        redirect(BASE_URL.'/install/import/11-04-00/step_1.php');
    }
}
if(!$cant_detect) {
