    ...
    <tbody>
        <?php if(count($records) > 0) { ?>
            <?php foreach($records as $key=>$record) { ?>
                <tr>
                    <td><a href="user_events.php?action=edit&id=<?php echo $record['id']; ?>"><?php echo $this->escape($record['title']); ?></td>
                    <td><?php echo $this->escape($record['date']); ?></td>
                    <td>
                        <a class="btn btn-default btn-xs" href="user_events.php?action=edit&id=<?php echo $record['id']; ?>"><span class="fa fa-pencil-square-o"></span> <?php echo $lang['user_edit']; ?></a>
                        <a class="btn btn-default btn-xs" href="<?php echo $record['url']; ?>"><span class="fa fa-eye"></span> <?php echo $lang['view']; ?></a>
                        <a class="btn btn-default btn-xs" onclick="return confirm('<?php echo $lang['messages_confirm']; ?>');" href="user_events.php?action=delete&id=<?php echo $record['id']; ?>"><span class="fa fa-times"></span> <?php echo $lang['user_delete']; ?></a>
                    </td>
                </tr>
            <?php } ?>
        <?php } else { ?>
