<?php if($form) { ?><form class="form-inline" id="table_list_form" name="table_list_form" method="post"><?php } ?>
    ...
    <div class="table_container">
        <table class="table table-striped table-bordered table-hover" summary="<?php echo $table_summary; ?>">
            <?php if(!$all_one_page) { ?>
                <caption>
                    ...
                    <span class="page-numbers">
                        <div class="btn-group">
                            <a class="btn btn-mini dropdown-toggle" data-toggle="dropdown" href="#"><?php echo $lang['admin_general_table_list_page']; ?> <span class="caret"></span></a>
            ...
            <thead>
                <tr>
                    <?php if($checkbox_options) { ?>
                        <script type="text/javascript">
                        $(document).ready(function() {
                            $("#table_list_checkall").click(function() {
                                var checked_status = this.checked;
                                $('#table_list_form input[name="table_list_checkboxes[]"]').each(function() {
                                    this.checked = checked_status;
                                });
                            });
                        });
                        </script>
                        <th scope="col" class="table-list-checkbox" nowrap><input type="checkbox" id="table_list_checkall"></th>
                    <?php } ?>
                    <?php if($sortable) { ?>
                    ...
                    <tr>
                        <th scope="row" colspan="<?php echo ($checkbox_options ? count($columns) + 1 : count($columns)); ?>">
                            ...
                            <?php foreach($checkbox_options AS $key=>$value) { ?>
                                ...
                                <?php if($key == 'select') { ?>
                                    <select class="select" name="<?php echo $value['name']; ?>" onchange="<?php echo $value['onchange']; ?>">
                            ...
                            <?php } ?>
                            <input class="btn" type="submit" value="<?php echo $lang['admin_submit']; ?>" name="table_list_submit" id="table_list_submit">
                        </th>
                    </tr>
                <?php } ?>
            </tfoot>
            <tbody<?php if($sortable) { ?> class="sortable"<?php } ?>>
                <?php if(count($records) > 0) { ?>
                    <?php foreach($records as $key=>$record) { ?>
                        <tr id="table_id_<?php echo $record['id']; ?>">
                            <?php if($sortable) { ?><td class="text-center"><i class="icon-resize-vertical handle"></i></td><?php } ?>
                            <?php if($checkbox_options) { ?>
                                <td><input class="table-list-checkbox-input" type="checkbox" name="table_list_checkboxes[]" value="<?php echo $record[$checkbox_value]; ?>"></td>
                            <?php } ?>
                ...
                <?php } else { ?>
                    <tr>
                        <th colspan="<?php echo ($checkbox_options ? count($columns) + 1 : count($columns)); ?>" scope="row"></th>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
        <?php if(!$all_one_page) { ?>
            <div class="row-fluid table_footer">
                <div class="span5">
                ...
                <div class="span5 pagination_show">
