        ...
        <?php if(count($records) > 0) { ?>
            <?php foreach($records as $key=>$record) { ?>
                <tr>
                    ...
                    <td><?php echo $this->escape($record['extension']); ?></td>
                    <td><?php echo $this->escape($record['title']); ?></td>
                    <td><?php echo $this->escape($record['description']); ?></td>
                    <td>
                        <a class="btn btn-default btn-xs" href="user_documents.php?action=edit&id=<?php echo $record['id']; ?>&listing_id=<?php echo $record['listing_id']; ?>"><span class="fa fa-pencil-square-o"></span> <?php echo $lang['user_edit']; ?></a>
                        <a class="btn btn-default btn-xs" href="user_documents.php?action=download&id=<?php echo $record['id']; ?>&listing_id=<?php echo $record['listing_id']; ?>"><span class="fa fa-download"></span> <?php echo $lang['user_documents_download']; ?></a>
                        <a class="btn btn-default btn-xs" onclick="return confirm('<?php echo $lang['messages_confirm']; ?>');" href="user_documents.php?action=delete&id=<?php echo $record['id']; ?>&listing_id=<?php echo $record['listing_id']; ?>"><span class="fa fa-times"></span> <?php echo $lang['user_delete']; ?></a>
                    </td>
                </tr>
            <?php } ?>
        <?php } else { ?>
