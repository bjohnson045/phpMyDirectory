        ...
        <?php if(count($records) > 0) { ?>
            <?php foreach($records as $key=>$record) { ?>
                <tr>
                    ...
                    <td>
                        <?php if($record['url_thumbnail']) { ?>
                            <a href="<?php echo $record['url']; ?>" class="image_group" title="<?php echo $this->escape($record['title']); ?>" rel="image_group"><img src="<?php echo $record['url_thumbnail']; ?>"></a>
                        <?php } ?>
                    </td>
                    <td><?php echo $this->escape($record['title']); ?></td>
                    <td><?php echo $this->escape($record['ordering']); ?></td>
                    <td>
                        <a class="btn btn-default btn-xs" href="user_images.php?action=edit&id=<?php echo $record['id']; ?>&listing_id=<?php echo $record['listing_id']; ?>"><span class="fa fa-pencil-square-o"></span> <?php echo $lang['user_edit']; ?></a>
                        <a class="btn btn-default btn-xs" onclick="return confirm('<?php echo $lang['messages_confirm']; ?>');" href="user_images.php?action=delete&id=<?php echo $record['id']; ?>&listing_id=<?php echo $record['listing_id']; ?>"><span class="fa fa-times"></span> <?php echo $lang['user_delete']; ?></a>
                    </td>
                </tr>
            <?php } ?>
        <?php } else { ?>
