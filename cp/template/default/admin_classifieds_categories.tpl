...
$(document).ready(function() {
    $("#categories_search").keyup(function () {
        ...
        if(categories_search > 2 || (categories_search > 0 && !isNaN($("#categories_search").val()))) {
            $.ajax({ data: ({ action: "admin_classifieds_categories_search", value: $("#categories_search").val()}), success:
                function(data){
    ...
    function expanders() {
        ...
        $(".collapsed,.expanded").click(function(event) {
            ...
            if($(this).hasClass("collapsed")) {
                var that = this;
                $.ajax({ data: ({ action: "admin_classifieds_categories_expand", value: $(this).attr('id')}), success:
                    function(data){
    ...
    var sortOnComplete = function(data) {
        ...
        if(data.percent == 100) {
            setTimeout(function() {
                hideLoadingMessage();
                window.location.replace("<?php echo BASE_URL_ADMIN; ?>/admin_classifieds_categories.php");
            },1000);
        } else {
    ...
    var sortStart = function(start,num) {
        ...
        $.ajax({ data: ({ action: "admin_classifieds_categories_sort", start: start, num: num }), success: sortOnComplete, dataType: "json"});
    };

    $("#category_sort").click(function(e) {
        e.preventDefault();
        if(confirm(<?php echo $this->escape_js($lang['messages_confirm']); ?>)) {
            sortStart(0,50);
        }
    });
    ...
    var exportOnComplete = function(data) {
        ...
        if(data.percent == 100) {
            setTimeout(function() {
                hideLoadingMessage();
                window.location.replace("<?php echo BASE_URL_ADMIN; ?>/admin_classifieds_categories.php?action=download");
            },1000);
        } else {
    ...
    var exportStart = function(start,num) {
        ...
        $.ajax({ data: ({ action: "admin_classifieds_categories_export", start: start, num: num }), success: exportOnComplete, dataType: "json"});
    };
