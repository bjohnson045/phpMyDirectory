...
var rebuildOnComplete = function(data) {
    ...
    if(data.percent == 100) {
        ...
        $("#status_percent").hide();
        addMessage("success","'.$PMDR->getLanguage('admin_maintenance_rebuild_search_index_done').'","status_container");
    } else {
...
var rebuildStart = function(start,num) {
    ...
    $.ajax({ data: ({ action: "admin_maintenance_rebuild_search_index", start: start, num: num, }), success: rebuildOnComplete, dataType: "json"});
};
...
$template_content->set('title',$PMDR->getLanguage('admin_maintenance_rebuild_search_index'));
